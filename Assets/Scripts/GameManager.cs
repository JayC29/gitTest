...
public class GameManager : MonoBehaviour
{
    ...
    public List<Vector2Int> currentMoveList;
    public List<Vector2Int> otherMoveList;
    public List<Vector2Int> kingGuardGrid;

    //the piece that is causing the check
    GameObject checkingGrid;

    GameObject kingFound;
    ...
    {
        if(currentMoveList.Contains(GridForPiece(checkingGrid)))
        {
            return false;
        }

        List<Vector2Int> kingMoves = getAllowedMoveList(kingFound);
        int numMoves = kingMoves.Count;
        foreach(Vector2Int move in kingMoves)
        {
            if(otherMoveList.Contains(move))
            {
                numMoves--;
            }
        ...
        Vector2Int kingLocation = GridForPiece(kingFound);
        Vector2Int checkerLocation = GridForPiece(checkingGrid);
        Debug.Log("CheckerLocation: " + checkerLocation.ToString() + " King location: " + kingLocation.ToString());
        int adderX = 1;
        int adderY = 1;
        Vector2Int temp = new Vector2Int(checkerLocation.x, checkerLocation.y);
        if (kingLocation.x < checkerLocation.x && kingLocation.y == checkerLocation.y)
        {
            Debug.Log("entered guard1");
            while(checkerLocation.x - adderX != kingLocation.x)
            {
                temp.x = checkerLocation.x - adderX;
                kingGuardGrid.Add(temp);
                adderX++;
            }
        } 
        else if(kingLocation.x > checkerLocation.x && kingLocation.y == checkerLocation.y)
        {
            Debug.Log("entered guard2");
            while (checkerLocation.x + adderX != kingLocation.x)
            {
                temp.x = checkerLocation.x + adderX;
                kingGuardGrid.Add(temp);
        ...
        {
            Debug.Log("entered guard3");
            while (checkerLocation.y - adderY != kingLocation.y)
            {
                temp.y = checkerLocation.y - adderY;
                kingGuardGrid.Add(temp);
                adderY++;
            }
        }
        ...
        int sizeOfGuard = kingGuardGrid.Count;

        foreach(Vector2Int guards in kingGuardGrid)
        {
            Debug.Log("guard: " + guards.ToString());
            if (!currentMoveList.Contains(guards))
            {
                sizeOfGuard--;
            }
            else
            {
                Debug.Log("found a guard: " + sizeOfGuard);
            }
    ...
    public IEnumerator swawpPlayers()
    {
        ...
        public IEnumerator Move(GameObject piece, Vector2Int gridPoint)
    {
        ...
        //if player still checked after move than they lost
        if (currentPlayer.isChecked)
        {
            Debug.Log(currentPlayer.name + " checking conditions");
            if (checkConditions())
            {
                Debug.Log(currentPlayer.name + " GAME OVER");
                isGameOver = true;
                winner = otherPlayer.name;
            }
            else
            {
                Debug.Log(currentPlayer.name + " is no longer checked");
                currentPlayer.isChecked = false;
            }

        }
        kingFound = otherPlayer.pieces.Find(x => (x.GetComponent(typeof(Piece)) as Piece).type == PieceType.King);
        List<Vector2Int> checkMoves = getAllowedMoveList(piece);

        Debug.Log("checkpoint 2");

        //Debug.Log("King at " + GridForPiece(kingFound));
        if (checkMoves.Contains(GridForPiece(kingFound)))
        {
            checkingGrid = piece;
            //other player checked;
            otherPlayer.isChecked = true;

            foreach(GameObject eachPiece in currentPlayer.pieces)
            {

                (eachPiece.GetComponent(typeof(Piece)) as Piece).checkMoveList = getAllowedMoveList(eachPiece);

            }

            yield return StartCoroutine(swawpPlayers());

            foreach (GameObject eachPiece in currentPlayer.pieces)
            {
                (eachPiece.GetComponent(typeof(Piece)) as Piece).checkMoveList = getAllowedMoveList(eachPiece);
            }
            //(currentPlayer.pieces[0].GetComponent(typeof(Piece)) as Piece).checkMoveList
            Debug.Log(otherPlayer.name + " has been checked!");
            yield return StartCoroutine(swawpPlayers());
        }
    ...
    public List<Vector2Int> getAllowedMoveList(GameObject movingPiece)
    {
        ...
        //Debug.Log("entrance1============================================ " + currentPiece.name + " has entered from spot " + GridForPiece(movingPiece));
